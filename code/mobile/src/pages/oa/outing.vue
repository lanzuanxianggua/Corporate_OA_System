<template>
  <view class="container">
    <view class="card">
      <text class="section-title">外出申请</text>

      <view class="form-item">
        <text class="form-label">目的地</text>
        <input class="form-input" v-model="form.destination" placeholder="请输入外出目的地" />
      </view>

      <view class="form-item">
        <text class="form-label">外出日期</text>
      ...
      <view class="form-item">
        ...
        <textarea class="form-textarea" v-model="form.reason" placeholder="请输入外出事由" :maxlength="200" />
      </view>
    </view>
...
import { ref } from "vue";
import { submitOuting } from "@/api/common";
...
const onDateChange = (e: any) => { form.value.outingDate = e.detail.value; };
...
const handleSubmit = async () => {
  const { destination, outingDate, startTime, endTime, reason } = form.value;
  if (!destination || !outingDate || !startTime || !endTime || !reason) {
