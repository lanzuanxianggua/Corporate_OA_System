...
INSERT INTO sys_permission (parent_id, perm_code, perm_name, perm_type, path, sort_order, status, create_by) VALUES
  (2, 'hr-training', '培训管理', 'MENU', '/hr/training', 5, 'ACTIVE', 'system');

-- 2) 业务权限
SET @parent = (SELECT id FROM sys_permission WHERE perm_code='hr-training' OR (path='/hr/training' AND perm_type='MENU') LIMIT 1);
INSERT INTO sys_permission (parent_id, perm_code, perm_name, perm_type, sort_order, status, create_by) VALUES
  (@parent, 'hr-training:course:list', '课程管理', 'BUTTON', 1, 'ACTIVE', 'system'),
  (@parent, 'hr-training:session:list', '班级管理', 'BUTTON', 2, 'ACTIVE', 'system');
