...
CREATE TABLE `oa_attendance` (
  ...
  `status`          CHAR(1)      NOT NULL DEFAULT '0'    COMMENT '状态(0正常 1迟到 2早退 3缺勤 4迟到且早退)',
...
CREATE TABLE `oa_notice` (
  ...
  `type`        CHAR(1)       NOT NULL DEFAULT '0'    COMMENT '类型(0通知 1公告 2制度)',
  `emp_id`      BIGINT        DEFAULT NULL            COMMENT '发布人ID',
  `status`      CHAR(1)       NOT NULL DEFAULT '0'    COMMENT '状态(0草稿 1已发布 2已撤回)',
...
CREATE TABLE `oa_schedule` (
  ...
  `start_time`  DATETIME     NOT NULL                COMMENT '开始时间',
  `end_time`    DATETIME     NOT NULL                COMMENT '结束时间',
  `status`      CHAR(1)      NOT NULL DEFAULT '0'    COMMENT '状态(0待办 1进行中 2已完成 3已取消)',
  `del_flag`    CHAR(1)      NOT NULL DEFAULT '0'    COMMENT '删除标志(0存在 1删除)',
  `create_by`   BIGINT       DEFAULT NULL            COMMENT '创建人',
  `create_time` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by`   BIGINT       DEFAULT NULL            COMMENT '更新人',
...
CREATE TABLE `oa_todo` (
  ...
  `emp_id`        BIGINT       NOT NULL                COMMENT '员工ID',
  `title`         VARCHAR(200) NOT NULL                COMMENT '标题',
  `todo_type`     CHAR(1)      NOT NULL                COMMENT '类型(0审批 1催办 2提醒)',
...
CREATE TABLE `oa_contract` (
  ...
  `contract_type` CHAR(1)        NOT NULL                COMMENT '合同类型(0采购 1销售 2服务 3劳务 4其他)',
...
CREATE TABLE `oa_approval_record` (
  ...
  `task_id`             BIGINT       DEFAULT NULL            COMMENT '任务ID',
  `node_index`          INT          DEFAULT NULL            COMMENT '节点索引',
  `action`              CHAR(1)      NOT NULL                COMMENT '操作(0通过 1驳回 2转办 3退回 4撤回)',
  `action_emp_id`       BIGINT       NOT NULL                COMMENT '操作人ID',
  `action_emp_name`     VARCHAR(64)  DEFAULT NULL            COMMENT '操作人姓名',
  `remark`              VARCHAR(500) DEFAULT NULL            COMMENT '审批意见',
