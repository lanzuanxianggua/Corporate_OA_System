<template>
  <div>
    ...
    <el-card>
      <template #header><span class="font-medium">文档管理</span></template>
      <el-table :data="docList" stripe>
        <el-table-column label="文档名称">
          <template #default="{ row }">
            <div class="flex items-center gap-2"><el-icon><Document /></el-icon><span>{{ row.docName }}</span></div>
          </template>
        </el-table-column>
...
const handleDelete = async (id: number) => {
  try { await ElMessageBox.confirm("确定删除？", "提示", { type: "warning" }); await deleteDocument(id); ElMessage.success("删除成功"); await fetchData(); } catch {}
};
