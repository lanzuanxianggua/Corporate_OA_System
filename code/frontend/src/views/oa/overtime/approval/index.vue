  ...
  <div class="h-full">
    ...
    <el-dialog v-model="dialogVisible" :title="approveAction === 1 ? '审批通过' : '审批拒绝'" width="500px" :close-on-click-modal="false">
      <template v-if="currentRow">
        ...
        <ApprovalTimeline v-if="currentRow?.id" business-type="overtime" :business-id="currentRow.id" />
...
const handleApprove = async () => {
  ...
  try {
    await approveOvertime({ id: currentRow.value.id, status: approveAction.value, remark: remark.value });
