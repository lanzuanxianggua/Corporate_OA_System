    ...
    <el-row :gutter="20" class="h-full">
        ...
        <el-card shadow="never" class="h-full">
          <template #header>
            <div class="flex items-center justify-between">
              <span class="text-base font-semibold text-[var(--oa-text)]">我的采购记录</span>
          ...
          <el-table max-height="calc(100vh - 330px)" :data="tableData" v-loading="loading" stripe style="width: 100%" size="small" :header-cell-style="{ background: 'var(--oa-surface-soft)', color: 'var(--oa-muted)' }">
            <template #empty>
              <el-empty description="暂无采购记录" :image-size="60" />
            </template>
      ...
      <el-col :xs="24" :md="14">
        <el-card shadow="never">
          <template #header>
            <span class="text-base font-semibold text-[var(--oa-text)]">提交采购申请</span>
          </template>
...
const handleExport = async () => {
  try {
    await downloadFile("/api/purchase/export", "采购数据.xlsx");
